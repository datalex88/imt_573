...
```{r Inspecting data}
#unique(flights$carrier) ## doublecheck what 00, 9E, and B6 means.
...
flight_time_wo <- flights %>%
  filter(!is.na(air_time), !is.na(air_time)) ## Removing NA values... these could potentiall be canceled flights.

min(flight_time_wo$air_time) ## Seeing a min of 20 min which could be weird.
